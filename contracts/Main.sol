...
contract Contract is IPinkLock, Pausable, Ownable {
    ...
    IERC20 private feeExemptionToken;
    uint256 public minimumTokenForFeeExemption;
    uint256 public extraBNB;
    ...
    // constructor(address _snipefinancemultisenders) {
        // snipefinancemultisenders = ISnipeFinanceMultisenders(
    //         _snipefinancemultisenders
    //     );
    //     lockFee = 0.00001 ether;
    //     vestFee = 0.00001 ether;
    // }
    constructor(address _snipefinancemultisenders, IERC20 _tokenAddressForFeeExemption, uint256 _minimumTokenForFeeExemption) {
        snipefinancemultisenders = ISnipeFinanceMultisenders(_snipefinancemultisenders);
        feeExemptionToken = _tokenAddressForFeeExemption;
        minimumTokenForFeeExemption = _minimumTokenForFeeExemption;
        lockFee = 0.00001 ether;
        vestFee = 0.00001 ether;
    }

    function setSnipeFinanceContract(address contractAddress)
        external
        onlyOwner
    {
    ...
    function LockBNB(
        ...
        // if (!snipefinancemultisenders.readAuthorizedUsers(msg.sender) && snipefinancemultisenders.getTokenBalance(msg.sender) < minimumTokenForFeeExemption) {
            ...
            require(
                ...
                "You have to pay fee for vesting"
            ); //TAKE FEE IF NOT AUTHORIZED
            feeamounts += lockFee;
            payable(receiver).transfer(lockFee);
        } else {
    ...
    // new code 16/JanF
    function unlockBNB(uint256 lockId, uint256 timestamp) external payable {
        require(lockId > 0, "invalid lock id");
        BNBLock storage userLock = _BNBlocks[_getActualIndexBNB(lockId)];
        require(timestamp >= userLock.tgeDate, "It is not time to unlock");
        require(
    ...
    // new code
    function VestingUnLockBNB(uint256 lockId, uint256 timestamp) external validLockBNB(lockId) {
        BNBLock storage userLock = _BNBlocks[_getActualIndexBNB(lockId)];
        require(
            userLock.owner == msg.sender,
            "You are not the owner of this lock"
        ...
        _vestingUnlockBNB(userLock, timestamp);
    }
    ...
    function unlock(uint256 lockId, uint256 timestamp) external override validLock(lockId) {
        ...
        if (userLock.tgeBps > 0) {
            _vestingUnlock(userLock, timestamp);
        } else {
            _normalUnlock(userLock, timestamp);
        }
    }

    function _normalUnlock(Lock storage userLock, uint256 timestamp) private {
        require(
            timestamp >= userLock.tgeDate,
    ...
    function _vestingUnlock(Lock storage userLock, uint256 timestamp) private {
        uint256 withdrawable = _withdrawableTokens(userLock, timestamp);
    ...
    // new code
    function _vestingUnlockBNB(BNBLock storage userLock, uint256 timestamp) private {
        uint256 withdrawable = _withdrawableTokensBNB(userLock, timestamp);
        uint256 newTotalUnlockAmount = userLock.unlockedAmount + withdrawable;
        require(
            withdrawable > 0 && newTotalUnlockAmount <= userLock.amount,
            "Nothing to unlock"
    ...
    function withdrawableTokens(uint256 lockId, uint256 timestamp)
        external
        view
        returns (uint256)
    {
        Lock memory userLock = getLockById(lockId);
        return _withdrawableTokens(userLock, timestamp);
    }

    function _withdrawableTokens(Lock memory userLock, uint256 timestamp)
        private
        pure
        returns (uint256)
    {
        ...
        if (timestamp < userLock.tgeDate) return 0;
        ...
        if (timestamp >= userLock.tgeDate) {
            currentTotal =
                (((timestamp - userLock.tgeDate) / userLock.cycle) *
                    cycleReleaseAmount) +
    ...
    // new code
    function withdrawableTokensBNB(uint256 lockId, uint256 timestamp)
        external
        view
        returns (uint256)
    {
        ...
        return _withdrawableTokensBNB(userLock, timestamp);
    }

    // new  code
    function _withdrawableTokensBNB(BNBLock memory userLock, uint256 timestamp)
        private
        pure
        returns (uint256)
    {
    ...
    function getBNBLockAt(uint256 index)
        external
        view
    ...
    // new code 17/Jan
    function getLockByIdBNB(uint256 lockId)
        public
        view
        returns (BNBLock memory)
    {
    ...
    // new code 17/Jan
    function BNBLockCountForUser(address user) public view returns (uint256) {
    ...
    // new code 18/Jan
    function totalLockCountForBNB() external view returns (uint256) {
    ...
    // new code 18/Jan
    function getLocksForBNB(uint256 start, uint256 end)
        external
        view
        returns (BNBLock[] memory)
    {
    ...
    // new code 17/Jan
    function _getActualIndexBNB(uint256 lockId)
        internal
        view
        returns (uint256)
    {
    ...
    function setReceiver(address _receiver) external onlyOwner {
        // require(_receiver != address(this), "Contract can not be receiver");
        // require(_receiver != address(0), "Address 0 can't be the reciver");
        receiver = _receiver;
    }
    ...
    //     feeExemptionToken = token;
    //     minimumTokenForFeeExemption = _amount;
    // }
    ...
    // To receive accidental and reward BNB against contract token holding
    receive() external payable {
        extraBNB += msg.value;
    }

    // function to withdraw accidental, reward BNB
    function withdrawBNB() external onlyOwner {
        require(extraBNB > 0, "no extra BNB available to withdraw");
        (bool success, ) = payable(receiver).call{value: extraBNB}("");
        require(success, "withdrawBNB failed");
    }
}